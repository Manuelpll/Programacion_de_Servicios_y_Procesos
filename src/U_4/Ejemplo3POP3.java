...
public class Ejemplo3POP3 {
	public static void main(String[] args) {
		...
		try {
			...
			if (!pop3SClient.login(username, password))
				...
				recuperarCabeceras(pop3MessageInfo, pop3SClient);
	...
	private static void recuperarMensajes(POP3MessageInfo[] pop3MessageInfo, POP3SClient pop3SClient)
			throws IOException {
		for (int i = 0; i < pop3MessageInfo.length; i++) {
			System.out.println("Mensaje: " + (i + 1));
			POP3MessageInfo msgInfo = pop3MessageInfo[i]; // lista de mensajes
			System.out.println("Identificador: " + msgInfo.identifier + ", Numero: " + msgInfo.number + ", Tamaño: "
					+ msgInfo.size);

			System.out.println("Prueba de listUniqueIdentifier: ");
			POP3MessageInfo pop3MInfo = pop3SClient.listUniqueIdentifier(i + 1);
			System.out.println("\tIdentificador: " + pop3MInfo.identifier + ", Numero: " + pop3MInfo.number
					+ ", Tamaño: " + pop3MInfo.size);
		} // for
	}// recuperarMensajes

	private static void recuperarCabeceras(POP3MessageInfo[] pop3MessageInfo, 
		...
		for (int i = 0; i < pop3MessageInfo.length; i++) {
			...
			BufferedReader bufferedReader = 
					(BufferedReader) pop3SClient.retrieveMessageTop(msgInfo.number, 0);
			String linea;
